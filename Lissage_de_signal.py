...
def main():
    ...
    for i in range(nbMesure - 1):
        difference = mesures[i] - mesures[i + 1]

        if difference < 0:
            difference = -difference

        if difference > diffMax:
            ok = False
    ...
    while ok == False:
        ...
        for i in range(nbMesure):
            if i == 0 or i == nbMesure - 1:
                nouvelle.append(mesures[i])
            else:
                moyenne = (mesures[i - 1] + mesures[i + 1]) / 2
                nouvelle.append(moyenne)
